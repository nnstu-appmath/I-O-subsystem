...
# USB
Разработано Intel, IBM, Microsoft 
Первые спецификации для USB 1.0 были представлены в 1994—1995 годах

Цели:
-упрощение добавления периферийных устройств благодаря стандартизации кабелей и процессу конфигурации программного обеспечения. 
![USB](https://digital-razor.ru/upload/iblock/68e/68e7eda4c6522629da97d8930c4fb637.jpg)
# PCI и PCI Express
PCI (англ. Peripheral component interconnect «взаимосвязь периферийных компонентов») — шина ввода-вывода для подключения периферийных устройств к материнской плате компьютера.
	Разработана Intel в 1994 году
Цели:
для добавления плат расширения(дополнительные порты последовательного ввода-вывода или USB-порты)
 сетевые интерфейсы
 звуковые карты, модемы, дисковые контроллеры или видеокарты. 
![PCI](https://upload.wikimedia.org/wikipedia/commons/thumb/c/ca/Pci-slots.jpg/640px-Pci-slots.jpg)

# PCI Express
![PCI Express](https://upload.wikimedia.org/wikipedia/commons/thumb/3/3e/PCI-E_%26_PCI_slots_on_DFI_LanParty_nF4_SLI-DR_20050531.jpg/274px-PCI-E_%26_PCI_slots_on_DFI_LanParty_nF4_SLI-DR_20050531.jpg)
...
![Память DDR)(https://2hpc.ru/wp-content/uploads/2015/11/big1293018085.png)

На Рис. изображен  DDR3 модуль памяти с двухрядным расположением выводов (DIMM). Этот модуль имеет по 120 контактов с двух сторон, что в сумме дает 240 контактов, включая 64-разрядную шину данных, 16-разрядную адресную шину
 В 2012 году модули DIMM обычно содержали от 1 до 16 ГБ оперативной памяти. Емкость памяти увеличивается вдвое примерно каждые 2–3 года.
...
# Сеть
![сеть](https://itumnik.ru/wp-content/uploads/2020/11/Kartinka-3.-Osnovnye-harakteristiki.jpg)
TCP/IP - это набор протоколов, который задает стандарты связи между компьютерами и содержит подробные соглашения о маршрутизации и межсетевом взаимодействии.
	Первые беспроводные технологии на базе Wi-Fi   датируются еще 1999 годом. Они носили название 802.11а, и позволяли развить скорость до 54 Мбит в секунду. С тех пор было принято еще около 20 наименований стандартов, которые вместе объединяются в 6 поколений. Сейчас активно разрабатывается Wi-Fi   технология седьмого поколения, выход которого запланирован на 2024 год.

# SATA
![SATA](https://upload.wikimedia.org/wikipedia/commons/thumb/2/29/SATA_ports.jpg/640px-SATA_ports.jpg)
(англ. Serial ATA) — последовательный интерфейс обмена данными с накопителями информации. 
Предназначен для подключения к шине устройств, жёстких дисков, оптических приводов, SSD накопителей и других.
Первая версия была представлена в 2000 году 
	Существует множество компьютерных продуктов, которые обозначаются как устройства SATA. ваш компьютер подключается через два порта SATA: один на диске, а другой — на материнской плате.
Несмотря на то, что SATA позиционируется одним портом, но состоит он из двух. Эти два порта называются разъем данных и разъем питания. Первый — это короткий 7-контактный разъем, а второй — более удлиненный 15-контактный.
Помимо длины, разъёмы можно различить по кабелям, которые к ним подключаются. Кабель для передачи данных SATA обычно состоит из твердого пластика, который переходит в плоский одножильный кабель, а разъем питания продолжается от его головки до нескольких тонких закругленных проводов разных цветов.

Оба кабеля необходимы для работы устройств SATA, и они выполняют разные задачи. 

Кабель для передачи данных обеспечивает высокоскоростное соединение с остальной частью компьютера, передавая информацию туда и обратно по запросу, а кабель питания — то, что в первую очередь обеспечивает диск энергией.
